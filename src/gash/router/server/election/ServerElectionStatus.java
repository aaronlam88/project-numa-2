...
=======
	private int currentTerm; // latest term server has seen (initialized to on
								// first boot, increases monotonically)
	private int votedFor; // candidateId that received vote in current term (or
							// null if none)
>>>>>>> 73bfcd0f437ab850d8eecbe7162fad919677cefe
	private int totalVotesRecievedForThisTerm;
	private ArrayList<Log> log; // each entry contains command for state
								// machine, and term when entry was received by
	...
	// to be increased after second message after append entry - after final
	// commit
	private int lastAplliedIndex; // index of highest log entry applied to state
	...
	private int nextIndex; // for each server, index of the next log entry to
							// send to that server (initialized to leader last
							// log index + 1)
	private int prevIndex; // for each server, index of highest log entry known
...
=======
	private int leaderId; // initialized to zero which means the server
							// initially has no leader and in a follower state,
							// tyring to conenct to leader
	private boolean electionTimeout = true;
	private boolean heartbeatTimeout = false;
>>>>>>> 73bfcd0f437ab850d8eecbe7162fad919677cefe
...
>>>>>>> 73bfcd0f437ab850d8eecbe7162fad919677cefe
	 ...
	 * 
	 * Log log =new Log(msg,term_num); this.lastTermInLog=term_num; return log;
	 * }
	 */

	/*
	 * public Log getLastLog(){ return log.get(log.size()-1); }
	 * 
	 * public void setLog(Log addlog){ if(this.log!=null){ this.log.add(addlog);
	 * } else{ log= new ArrayList<Log>(); } }
	 */
